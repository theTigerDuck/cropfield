 ...
 * @subpackage Form
 */
class CropField extends FileField {
     ...
     * @var string
     */
    protected $previewTemplate = 'TheTigerDuck\\Cropfield\\FileAttachmentField_cropablepreview';
    ...
    {
        if($this->isCMS()) {
            Requirements::javascript('thetigerduck/cropfield:js/file_attachment_field_backend.js');
        }
        Requirements::css('thetigerduck/cropfield:css/file_attachment_field.css');
    ...
    public function setValue($val, $data = array())
    {
        ...
        if ($data && is_array($data) && isset($data[$this->getName()])) {
            ...
            if (is_array($data[$this->getName()])) {
                ...
                foreach ($ids as $i => $id) {
                    if ($i == "posX" || $i == "posY" || $i == "width" || $i == "height" || $i == "rotate") {
                        unset($ids[$i]);
                        $val = $ids;
                    }
                    /*
                    if ($i != "poxX" && $i != "poxY" && $i != "width" && $i != "height" && $i != "rotate" && $validIDs && !isset($validIDs[$id])) {
                        unset($ids[$i]);
                        $isInvalid = true;
                    }*/
                }
    ...
    public function setView($view)
    {
        if(!in_array($view, array ('grid','list'))) {
            throw new Exception("CropField::setView - View must be one of 'grid' or 'list'");
        }
        $this->view = $view;
        return $this;
    }
    ...
    public function setPermissions($perms)
    {
        foreach($perms as $perm => $val) {
            if(!isset($this->permissions[$perm])) {
                throw new Exception("CropField::setPermissions - Permission $perm is not allowed");
            }
    ...
    private function getUploadUserError($code)
    {
        ...
        switch($code) {
            ...
            case UPLOAD_ERR_FORM_SIZE:
                $error_message = _t('CropField.ERRFILESIZE', 'The file is too large, please try again with a smaller version of the file.');
                break;
            case UPLOAD_ERR_PARTIAL:
                $error_message = _t('CropField.ERRPARTIALUPLOAD', 'The file was only partially uploaded, did you cancel the upload? Please try again.');
                break;
            case UPLOAD_ERR_NO_FILE:
                $error_message = _t('CropField.ERRNOFILE', 'No file upload was detected.');
                break;
            case UPLOAD_ERR_NO_TMP_DIR:
            case UPLOAD_ERR_CANT_WRITE:
            case UPLOAD_ERR_EXTENSION:
                $error_message = _t('CropField.ERRSYSTEMFAIL', 'Sorry, the system is not allowing file uploads at this time.');
                break;
            default:
                // handles if an extra error value is added at some point as a general error
                $error_message = _t('CropField.ERRUNKNOWNCODE', 'Sorry, an unknown error has occured. Please try again later.');
                break;
        }
    ...
    public function upload(HTTPRequest $request)
    {
        ...
        if ($this->isDisabled() || $this->isReadonly() || !$this->CanUpload()) {
            $error_message = _t('CropField.UPLOADFORBIDDEN', 'Files cannot be uploaded via this form at the current time.');
        ...
        if (empty($files) || empty($value)) {
            $error_message = _t('CropField.NOFILESUPLOADED', 'No files were detected in your upload. Please try again later.');
        ...
        if ($form) {
            ...
            if (!$token->checkRequest($request)) {
                $error_message = _t('CropField.BADSECURITYTOKEN', 'Your form session has expired, please reload the form and try again.');
                return $this->httpError(400, $error_message);
            }
        }
        ...
        foreach ($tmpFiles as $tmpFile) {
            ...
            try {
                ...
                $error_message = _t('CropField.GENERALUPLOADERROR', 'Sorry, the file could not be saved at the current time, please try again later.');
                return $this->httpError(400, $error_message);
            }
    ...
    public function handleSelect(HTTPRequest $request)
    {
        ...
        return CropField_SelectHandler::create($this, $this->getFolderName());
    }
    ...
    public function RootThumbnailsDir()
    {
        return $this->getSetting('thumbnailsDir') ?:
            ModuleResourceLoader::singleton()->resolveURL('thetigerduck/cropfield:images/file-icons');
    }
    ...
    {
        $file_path = ModuleLoader::inst()->getManifest()->getModule('thetigerduck/cropfield')
            ->getResource($this->config()->default_config_path)
            ->getPath();
        if(!file_exists($file_path)) {
            throw new Exception("CropField::getDefaults() - There is no config json file at $file_path");
        }
